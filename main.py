...
class AudioClassifier:
    ...
    def load_model(self):
        ...
        checkpoint = torch.load("/models/best_model.pt", map_location=self.device)
    ...
    def inference(self, request: InferenceRequest):
        ...
        spectrogram = self.audio_processor.process_audio_chunks(audio_data)
        ...
        with torch.no_grad():
            ...
            probabilities = torch.softmax(output,
                                          dim=1)  # dim =0 batch, dim=1 class(batch size, num_classes)
            top3_probabilities, top3_labels = torch.topk(probabilities, 3)
            top3_probabilities = top3_probabilities.squeeze(0)
            top3_labels = top3_labels.squeeze(0)

            predictions = [{"class": self.classes[idx.item()], "confidence": prob.item()} for
                           prob, idx in
                           zip(top3_probabilities, top3_labels)]
...
def main():
    ...
    if waveform_info:
        ...
        print(
            f"Sample Rate: {waveform_info.get('sample_rate')}, Duration: {waveform_info.get('duration'):.2f} seconds")

    print("Top predictions")

    for prediction in result.get("predictions", []):
        print(f"Class: {prediction['class']}, Confidence: {prediction['confidence']:.4f}")